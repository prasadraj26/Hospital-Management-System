...
function AdminNurse() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:4451/api/admin/get-department"
        );
  ...
  const getNurses = async () => {
    try {
      const response = await axios.get("http://localhost:4451/api/nurse/get-nurses");
  ...
  const handleAddNurse = async (e) => {
    e.preventDefault();
     await axios
      .post("http://localhost:4451/api/nurse/add-nurse", {
        name: nurname,
        email: nuremail,
        department: nurdept,
      ...
      <div className="w-[95%] max-w-6xl bg-white shadow-xl flex flex-col md:flex-row overflow-hidden rounded-xl border border-navy-100 min-h-[80vh]">
        ...
        <div className="w-full md:w-[75%] p-4 md:p-6 flex flex-col justify-start gap-5">
          ...
          <button
            ...
            className="bg-navy-700 p-2 w-[10%] rounded-full hover:scale-110 duration-200 active:scale-90 text-white"
          >
            Create
          </button>
        </div>
