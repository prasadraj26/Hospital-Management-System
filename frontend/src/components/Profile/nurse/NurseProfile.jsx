...
function NurseProfile() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      axios
        .put("http://localhost:4451/api/nurse/profile-update", {
          userId: userData._id,
          updatedProfile: {
            email: email,
            name: name,
          ...
          <form action="" className="flex flex-col h-[80%] justify-between">
            ...
            <div className="w-full flex space-x-2">
              <button
                ...
                className="bg-black flex-1 text-white p-2 rounded-full"
              >
                Update
              </button>
              <button
                onClick={() => setShowPasswordModal(true)}
                className="bg-blue-500 hover:bg-blue-700 text-white p-2 rounded-full px-4"
              >
                Change Password
              </button>
            </div>
