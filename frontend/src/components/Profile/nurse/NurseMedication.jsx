...
function NurseMedication() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4451/api/user/get-users");
  ...
  const handleAddMedication = async(e) =>{
    e.preventDefault() ;
    await axios.post(`http://localhost:4451/api/user/add-medications/${changePatient}`,{name ,  frequency , dosage})
    .then((res) =>{
      Swal.fire({
        title: "Success",
        icon: "success",
